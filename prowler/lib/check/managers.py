...
class ExecutionManager:
    def __init__(
        ...
        self.check_dependencies = get_dependencies_for_checks(
            global_provider.type, self.check_dict
        )
    ...
    def import_client(self, client_name):
        if not self.loaded_clients.get(client_name):
            ...
            client_module = importlib.import_module(
                f"prowler.providers.{self.global_provider.type}.services.{module_name}.{client_name}"
            )
    ...
    def execute_checks(self) -> list:
        ...
        if self.global_provider.output_options.only_logs:
            ...
            if self.mutelist_file:
                print(
                    f"{Style.BRIGHT}Using the following Mute List: {Style.RESET_ALL}{Fore.YELLOW}{self.mutelist_file}{Style.RESET_ALL}\n"
                )
            # Default execution
    ...
    def execute(
        ...
        try:
            # Import check module
            check_module_path = f"prowler.providers.{self.global_provider.type}.services.{service}.{check_name}.{check_name}"
            ...
            # Run check
            check_findings = self.run_check(c, self.global_provider.output_options)
            ...
            # Mutelist findings
            if (
                hasattr(self.global_provider, "mutelist")
                and self.global_provider.mutelist
            ):
            ...
            report(check_findings, self.global_provider)
            ...
            if os.environ.get("PROWLER_REPORT_LIB_PATH"):
                try:
                    ...
                    custom_report_interface(
                        check_findings,
                        self.global_provider.output_options,
                        self.global_provider,
                    )
                except Exception:
                    sys.exit(1)
    ...
    @staticmethod
    def run_check(check: Check, output_options: Any) -> list:
        findings = []
    ...
    def update_audit_metadata(self):
        ...
        """
        try:
            self.global_provider.audit_metadata.services_scanned = len(
                self.services_executed
            )
            self.global_provider.audit_metadata.completed_checks = len(
                self.checks_executed
            )
            self.global_provider.audit_metadata.audit_progress = (
                100
                * len(self.checks_executed)
                / len(self.global_provider.audit_metadata.expected_checks)
            )
        except Exception as error:
            logger.error(
                f"{error.__class__.__name__}[{error.__traceback__.tb_lineno}]: {error}"
